...
class NeuralNet:
    ...
    def train(
        ...
        for i in range(print_count):
            ...
            eta = (iters - count) * total/print_interval/60
            # one extra iteration of `one_pass_with_error`
            count += 1
            print(f"Error after {count} iterations: {one_pass_with_error()} ETA: {round(eta, 2)} minutes")
    ...
    def back_propagate(
        ...
        # ADDED CODE
        entropy = -sum(out * math.log(out + 1e-8) for out in self.output_layer)
        # Lower entropy means output is peaky or uniform → punish if it's too low
        entropy_penalty = max(0, 1.2 - entropy)  # you can tune 1.5 as a target entropy
        square_errors += 0.1 * entropy_penalty
